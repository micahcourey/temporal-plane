...
Mnemix is a standalone, local-first memory layer for AI coding agents and related tooling.
...
Mnemix is **not** tied to one agent framework. It should be usable from multiple hosts, with AI DX Toolkit as only the first adapter.

---

## 2. Product Goals

Mnemix v1 should provide:
...
Mnemix v1 should **not** attempt to be:
...
Mnemix should position itself as:
...
- **Letta**, which is strong on stateful agent abstractions and memory architecture, but broader and more runtime-centric than needed for Mnemix v1

Mnemix should borrow the best product ideas from both while staying materially simpler.
...
Mnemix should primarily store durable, useful memories, not unbounded raw transcripts.
...
Mnemix should use **LanceDB + Lance** as its storage foundation.
...
Mnemix should make it easy to answer:
...
Mnemix should preserve that distinction clearly in its API and CLI.

---

## 11. New Lance Version-Control Findings and Their Impact

Recent Lance capabilities materially strengthen the Mnemix roadmap.

### 11.1 Tags should be first-class in v1

Lance supports immutable, named references to versions.

Mnemix should use this capability for:
...
Mnemix should not ignore this in the architecture, even if branch UX is deferred.
...
Mnemix should keep this in mind for future:
...
Mnemix should store durable memory objects rather than unstructured transcript logs.
...
Mnemix should support scoped memory boundaries.
...
Progressive disclosure should be a defining feature of Mnemix.
...
Mnemix should expose not just retrieval results, but also enough metadata to explain:
...
Mnemix should define its own product-level abstractions over Lance versioning.
...
Before destructive cleanup, Mnemix should encourage or create a checkpoint.
...
Mnemix should expose useful observability without requiring a heavy dashboard.
...
- Mnemix should treat tags/checkpoints as first-class v1 product concepts.
- Mnemix should be architecturally branch-aware from day one.
- Mnemix should likely defer explicit branch/shallow-clone UX until after v1.
- Mnemix should default to conservative retention because cleanup can remove recoverability.
...
Mnemix should be built as a standalone Rust memory engine backed by LanceDB/Lance, with:
...
**Mnemix should ship v1 with serious version-awareness and checkpointing, while staying architecturally ready for true branch-based memory workflows later.**
