...
class TestFindBinary:
    def test_env_var_used_when_set(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("TP_BINARY", "/custom/tp")
        envelope = {"kind": "stats", "data": {"stats": {}}}
        with patch("subprocess.run", return_value=_make_result(json.dumps(envelope))):
            # Should not raise
            with patch("temporal_plane._runner._find_binary", return_value="/custom/tp"):
                pass  # just verify it's used by testing the run() path below
...
class TestRunDecoding:
    ...
    def test_returns_data_from_envelope(self) -> None:
        ...
        with patch("shutil.which", return_value="/usr/bin/tp"), \
    ...
    def test_correct_args_passed_to_subprocess(self) -> None:
        ...
        assert call_args == ["/usr/bin/tp", "--store", str(_STORE), "--json", "init"]
